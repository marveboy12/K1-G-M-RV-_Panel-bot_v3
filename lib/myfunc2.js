...
exports.buffergif = async (image) => {
	...
	const filename = `${Math.random().toString(36)}`
			await fs.writeFileSync(`./C-SMedia/trash/${filename}.gif`, image)
					 child_process.exec(
								`ffmpeg -i ./C-SMedia/trash/${filename}.gif -movflags faststart -pix_fmt yuv420p -vf "scale=trunc(iw/2)*2:trunc(ih/2)*2" ./C-SMedia/trash/${filename}.mp4`
										) 
  await sleepy(4000)
  
	var buffer5  =  await  fs.readFileSync(`./C-SMedia/trash/${filename}.mp4`)
	Promise.all([unlink(`./XeonMedia/video/${filename}.mp4`), unlink(`./C-SMedia/gif/${filename}.gif`)])
	return buffer5
				   }